...
import { FaEdit, FaCheck, FaEnvelope, FaPhone, FaMapMarkerAlt, FaUser } from "react-icons/fa";
...
function Profile() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        // 429 — rate limit hit, don't treat as session expired
        if (res.status === 429) {
          Swal.fire({
        ...
        if (userInfo.firstname) userInfo.firstname = capitalizeName(userInfo.firstname);
        if (userInfo.lastname)  userInfo.lastname  = capitalizeName(userInfo.lastname);
  ...
  const handleUpdate = async () => {
    try {
      ...
      // 429 — rate limit hit during update
      if (res.status === 429) {
        Swal.fire({
          title: "Too Many Requests",
          text: "Slow down! Try again in a minute.",
      ...
      if (updatedUser.firstname) updatedUser.firstname = capitalizeName(updatedUser.firstname);
      if (updatedUser.lastname)  updatedUser.lastname  = capitalizeName(updatedUser.lastname);
  ...
  // Guard — user null (429 or unexpected error, no redirect)
  if (!user) {
    return (
      <div className="flex h-screen items-center justify-center bg-gray-100">
        <div className="text-center">
          ...
          <button
            onClick={() => { fetchedRef.current = false; setLoading(true); }}
            ...
            <div className="p-8">
              <div className="space-y-4">
                ...
                <div className="border-b pb-3">
                  ...
                  {editing ? (
                    <input type="tel" name="contactno" value={form.contactno || ""} onChange={handleChange}
                      className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div className="border-b pb-3">
                  ...
                  {editing ? (
                    <textarea name="address" value={form.address || ""} onChange={handleChange}
                      rows="3" className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
